...
class Solution {
 ...
 private:
    ...
    bool isValid(TreeNode* root, int low, int high) {
        ...
        int highbound = val == neg ? neg : val - 1;
        int lowbound = val == pos ? pos : val + 1;
        return isValid(root->left, low, highbound) &&
            isValid(root->right, lowbound, high);
    }
};
// @lc code=end

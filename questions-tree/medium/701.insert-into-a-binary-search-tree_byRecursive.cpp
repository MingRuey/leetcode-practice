...
class Solution {
 ...
 private:
    int target;
    bool done;
    void traverse(TreeNode* node, int lowbnd, int upbnd) {
        if (!node || done) return;

        traverse(node->left, lowbnd, node->val);
        traverse(node->right, node->val, upbnd);

        if (!node->left) {
            if (target > lowbnd && target < node->val) {
        ...
        if (!node->right) {
            if (target > node->val && target < upbnd) {

...
class Solution {
 ...
 private:
    ...
    void dfs(TreeNode* root, int level, int idx, int halfwidth) {
        if (!root) return;
        dfs(root->left, level + 1, idx - halfwidth/2 - 1, halfwidth/2);
        dfs(root->right, level + 1, idx + halfwidth/2 + 1, halfwidth/2);
        ans[level][idx] = to_string(root->val);
    }
};
// @lc code=end
